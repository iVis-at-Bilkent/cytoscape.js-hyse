...
// assumes edges are always from small level to large levels
function insertDummyNodesIfNeeded() {
  ...
  for (let i = 0; i < edges.length; i++) {
    ...
    const src = e.source();
    const tgt = e.target();
    let l1 = Number(src.data("level"));
    const l2 = Number(tgt.data("level"));
    let currSrc = src;
    while (l2 - l1 > 1) {
      // add dummy nodes and edges
      const id = "n" + Math.floor(Math.random() * 1000000000) + "_dummy";
      l1++;
      const newDummy = cy.add({
        data: { id: id, level: l1 },
        classes: "dummy",
      });
      cy.add({ data: { source: currSrc.id(), target: newDummy.id() } });
      currSrc = newDummy;
    }
    e.remove();
    cy.add({ data: { source: currSrc.id(), target: tgt.id() } });
  }
}
...
function exportSif() {
  ...
  const edgelessNodes = cy
    .nodes()
    .filter((x) => x.connectedEdges().length == 0);
  ...
  const _parse = function (line, i, links, nodes) {
    ...
    if (line.length < 1) {
      ...
      for (let j = 2; j < line.length; j++) {
        ...
        if (source < target) {
          links[source.id + target.id + edgeType] = {
            target: target.id,
            source: source.id,
            edgeType: edgeType,
          };
        } else {
          links[target.id + source.id + edgeType] = {
            target: target.id,
            source: source.id,
            edgeType: edgeType,
          };
